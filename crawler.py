...
def main():
  ...
  parser.add_argument('-t', '--target', help='Target files to crawl test / all / specific', choices=['test','all','specific'], required=True)
  parser.add_argument('-s', '--site', help='Specific site to be crawled if the target is set to specific')
  parser.add_argument('-r', '--rules', help='Rules for specific link decoration to be blocked, given as a comma separated list')
...
def crawl_specific_from_csv(csv_file_name):
  ...
  with open(csv_file_name, 'r') as f:
    ...
    count = 0
    for (_, site_url, decoration_name) in csv_reader:
      if count == 0:
        count += 1
        continue
...
def check_for_block_in_log(path, specific_folder):
    if os.path.isdir(f'{path}/{specific_folder}') and log_name in os.listdir(path):
      with open(log_name, 'rb', 0) as f, mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as s:
        if s.find(b'---------------------------------------------------') != -1:
          return True
...
def crawl_site(data_folder_name, log_name):
      ...
      while cur_attempts < 5 and not crawl_succeeded:
        cur_attempts += 1
        print(f'{tld_string} - Attempt {cur_attempts} / 5 - On folder {specific_folder_name}')

        rules_string = ','.join(rules)
        os.system(f'npm run crawl -- -u \"{url_to_crawl}\" -o {os.getcwd()}/{specific_folder_name} -v -f -q specific -s \'{rules_string}\' >> {os.getcwd()}/{log_name}')
        files  = os.listdir(f'{os.getcwd()}/{specific_folder_name}')

        crawl_succeeded = check_for_block_in_log(os.getcwd(), data_folder_name)
        for file in files:
          if tld_string in file:
            with open(log_name,'r') as f, mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as s:
              if s.find(b'---------------------------------------------------') != -1:
...
def crawl_specific_site_conductor(url_and_rule_to_crawl):
  ...
  if('' in rules and len(rules) == 1):
    ...
    while not cur_folder and cur_attempts < 5:
      # print(os.getcwd())
      ...
      # print(folder_name)
      os.system(f'npm run crawl -- -u \"{url_to_crawl}\" -o {os.getcwd()}/{folder_name} -v -f -q none >> {os.getcwd()}/{log_name}')
      try:
        files  = os.listdir(f'{os.getcwd()}/{folder_name}')
    ...
    try:
      ...
      if os.path.isdir(f'{os.getcwd()}/{specific_folder_name}') and log_name in os.listdir(os.getcwd()):
        with open(log_name, 'rb', 0) as f, mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as s:
          if s.find(b'---------------------------------------------------') != -1:
            crawl_succeeded = True
  ...
  while cur_control_folder < 5 and cur_control_attempts < 12:
    # print(os.getcwd())
    print(f'{tld_string} - Attempt {cur_control_attempts} / 12 - On folder {cur_control_folder}')
...
# 
  ...
  for method in methods.keys():
    ...
    while cur_attempts < 2 and not methods[method]:
      cur_attempts += 1
      os.system(f'npm run crawl -- -u \"{url_to_crawl}\" -o ./{folder_name} -v -f -q {method} >> log')
  ...
  if run_control_on_cur_site:
    ...
    while cur_control_folder < 5 and cur_control_attempts < 25:
      print(f'{tld_string} - Attempt {cur_control_attempts} / 20 - On folder {cur_control_folder}')
      if cur_control_folder == 0 and cur_control_attempts == 5:
        break
      ...
      crawls_since_last_success += 1
      folder_name = f'control-data-{cur_control_folder}'
      os.system(f'npm run crawl -- -u \"{url_to_crawl}\" -o ./{folder_name} -v -f -q none >> log')
      files  = os.listdir(f'./{folder_name}')
      for file in files:
        if tld_string in file:
          cur_control_folder += 1
